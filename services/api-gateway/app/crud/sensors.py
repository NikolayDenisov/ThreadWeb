...
def create_sensor(db: Session, sensor: schema_sensor.SensorCreate):
    db_sensor = models_sensors.Sensor(id_type=sensor.id_type,
                                      id_owner=sensor.id_owner,
                                      code=sensor.code,
                                      name=sensor.name,
...
def get_sensor_by_code(db: Session, code: str):
    return db.query(models_sensors.Sensor).filter(
        models_sensors.Sensor.code == code).first()
...
def create_sensor_type(db: Session,
                       sensor_type: schema_sensor.SensorTypeCreate):
    db_sensor_type = models_sensors.SensorType(name=sensor_type.name,
...
def create_sensor_group(db: Session,
                        sensor_group: schema_sensor.SensorGroupCreate):
    db_sensor_group = models_sensors.SensorGroup(id_type=sensor_group.id_type,
                                                 code=sensor_group.code,
...
def create_measured_value(db: Session,
                          measured_value: schema_sensor.MeasuredValueCreate):
    db_measured_value = models_sensors.MeasuredValue(
        id_sensor=measured_value.id_sensor, date_measured=datetime.now(),
...
def create_sensor_group_members(db: Session,
                                sensor_group_members: schema_sensor.SensorGroupMembersCreate):
    db_sensor_group_members = models_sensors.SensorGroupMembers(
        group_id=sensor_group_members.group_id,
...
def get_values(db: Session, skip: int = 0, limit: int = 100):
    return db.query(models_sensors.MeasuredValue).offset(skip).limit(
        limit).all()
